...
fun PendingPairingScreen(pairing: PairingInfo, loading: Boolean, onPair: (String) -> Unit, onUseAnotherQr: () -> Unit) {
    ...
    var passphrase by remember(pairing) { mutableStateOf("") }
    ...
    Button(onClick = { onPair(passphrase) }, enabled = passphrase.isNotBlank() && !loading, modifier = Modifier.fillMaxWidth()) { Text(if (loading) "Vinculando…" else "Vincular") }
